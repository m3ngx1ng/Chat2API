...
<body>
  ...
  <script>
    ...
    function renderModels(payload){clearNode(modelsList);const models=(payload.models||[]);if(!models.length){modelsList.appendChild(empty('当前没有模型探测结果。'));setModels('尚未探测','warn');return}models.forEach(model=>{const row=document.createElement('div');row.className='row-item';const left=document.createElement('div');left.innerHTML='<div style="font-weight:800">'+model.id+'</div><div style="font-size:12px;color:var(--muted);margin-top:4px">'+(model.available?'可用':'不可用')+(model.reason?(' | '+model.reason):'')+'</div>';const badge=document.createElement('div');badge.className='status status-inline '+(model.available?'ok':'warn');badge.textContent=model.available?'可用':'不可用';row.append(left,badge);modelsList.appendChild(row)});const ts=formatUnix(payload.updated_at);setModels('已探测 '+String(payload.available||0)+' / '+String(payload.total||models.length)+' 个可用模型'+(ts?' | 更新时间 '+ts:''),'ok')}
    ...
    async function loadModels(){try{const r=await fetch('/admin/api/models',{credentials:'same-origin'});renderModels(await parseResponse(r))}catch(e){if(String(e.message).includes('401')){setModels('登录后可查看探测结果','warn');return}setModels(e.message,'warn')}}
    ...
    async function probeModels(){setModels('正在探测模型...');try{const r=await fetch('/admin/api/models/probe',{method:'POST',credentials:'same-origin'});renderModels(await parseResponse(r))}catch(e){setModels(e.message,'warn')}}
    ...
    $('loginBtn').onclick=login;$('logoutBtn').onclick=logout;$('saveBtn').onclick=saveConfig;$('addAuthBtn').onclick=()=>addAuth('');$('addPrefixBtn').onclick=()=>addPrefix('');$('addAccountBtn').onclick=()=>addAccount({});$('importBtn').onclick=importBulk;$('probeModelsBtn').onclick=probeModels;$('password').addEventListener('keydown',e=>{if(e.key==='Enter')login()});loadConfig();loadModels();ensureEmpty();
  </script>
</body>
</html>`
